    ...
    <div className="my-20">
      ...
      </div>
      {
        histories.map((history:donationProp, index:number) => {
          const charityIndex = charities.findIndex((item:charityProp) => item.wallet_address.toLowerCase() === history.to.toLowerCase());
          ...
          const logoImg = charityIndex >= 0 && charities[charityIndex].photo == '' ? defaultImg : "https://ipfs.io/ipfs/" + charities[charityIndex].photo;
          return (
            <Grid container spacing={1} key={index} className="border-b-1 p-5 ">
              <Grid item xs={6} className="flex items-center">
                <img src={logoImg} className="w-40 h-40 mr-10"/>
                <div>
