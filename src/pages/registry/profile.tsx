...
export const ProfilePage = () => {
  ...
  const loginUser:import("../../core/interfaces/base").loginUserProp = useSelector((state:any) => state.app.loginUser);
  const charityType = loginUser.charity_type == 0 ? 'charity' : 'fundraiser';
  const style = {
    label: 'text-18 my-15',
      ...
      <div className="w-[95%] md:w-[80%] mx-auto border-1 my-70 px-35 py-50">
        <form className="w-full" onSubmit={formik.handleSubmit}>
            ...
            <Grid item container xs={12} sm={8} md={9} spacing={1}>
              <div className="p-20 rounded-10 border-1 w-full">
                <div className="flex items-center">
                  <img src={"https://ipfs.io/ipfs/" + loginUser.photo} className="rounded-full w-100 h-100 border-1 mr-20"/>
                  <div>
                      <p className="text-24 font-bold">{loginUser.name}</p>
                      <p className="text-16">Update your photo and personaal details</p>
                  </div>
                </div>
          ...
          <Grid container spacing={1}>
            ...
            <Grid item xs={12} sm={6}>
              <button type="submit" className={baseStyles.greenBtn}>Update <FontAwesomeIcon icon={faArrowRight} /></button>
            </Grid>
          </Grid>
        </form>
      </div>
